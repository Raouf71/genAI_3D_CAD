...
async def main():
    config = RAGAnythingConfig(
        working_dir="./rag_storage_ollama_lager",
        parser="mineru",
        parse_method="txt",
    ...
    await rag.process_document_complete(
        file_path="C:/Users/grabe_stud/llm_cad_projekt/RAG/llm_3d_cad/RAG-Anything/external_knowledge_base/lager.pdf",
        output_dir="./output_ollama_lager",
        parse_method="txt",
    ...
    for d in range(20, 85, 5):    
        query = (
            "From the table with dimensions for Rillenkugellager DIN 625-1, "
            f"please extract the full row for d={d} teeth"
        )
        ...
        try:
            ...
            # Store each result, prefixed with the d value
            table_lookup_results.append(f"===== d = {d} =====\n{text_result}\n\n")
    ...
    output_file = "table_lookups_rillenkugellager.txt"
