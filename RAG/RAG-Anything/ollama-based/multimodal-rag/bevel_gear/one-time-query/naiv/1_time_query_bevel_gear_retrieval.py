...
async def main():
    config = RAGAnythingConfig(
        working_dir="./rag_storage_bevel_gear",
        parser="mineru",
        parse_method="auto",
    ...
    await rag.process_document_complete(
        file_path="C:/Users/grabe_stud/llm_cad_projekt/RAG/llm_3d_cad/RAG-Anything/external_knowledge_base/bevel_gear.pdf",
        output_dir="./output_bevel_gear",
        parse_method="auto",
    ...
    for m in range(10, 40, 5):
        ...
        query = (
            "From the table with dimensions for bevel gears made of zinc (ZnAl4Cu1), "
            f"please extract the full table row where Art.-Nr. = KD{m}16-1:1ZN",
        )
        ...
        try:
            ...
            # Store each result, prefixed with the module(m) value
            table_lookup_results.append(f"===== module = {m/10} =====\n{text_result}\n\n")
    ...
    output_file = "table_lookups_bevel_gear.txt"
    with open(output_file, "w", encoding="utf-8") as f:
        f.writelines(table_lookup_results)

    # User query (single)   
    # text_result = await rag.aquery(
    #     "From the table with dimensions for bevel gears made of zinc (ZnAl4Cu1), please extract the full table row where Art.-Nr. = KD1016-1:1ZN",
    #     vlm_enhanced=False,
    #     enable_rerank=False,
    #     # chunk_top_k=6,        
    #     mode="naive",
