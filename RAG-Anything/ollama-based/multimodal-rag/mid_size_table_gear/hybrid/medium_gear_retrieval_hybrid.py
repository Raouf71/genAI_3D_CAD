...
async def main():
    ...
    async def ollama_llm_model(
        ...
        **kwargs,
        ...
        if system_prompt and "---Context---" in system_prompt:
            ...
            logger.debug(f"[retrieval-debug] ===== FULL CONTEXT SECTION (as sent by LightRAG) =====")
            logger.debug(f"{ctx_section}")
            logger.debug(f"[retrieval-debug] ===== END CONTEXT SECTION =====")
            # ===================== RETRIEVAL DEBUG END =======================
    ...
    text_result = await rag.aquery(
        "From the table with dimensions for spur gears, please extract the full row where Number OfTeeth=1M57 Teeth ",
        vlm_enhanced=False,
        enable_rerank=False,
        chunk_top_k=6,        
        mode="hybrid",
