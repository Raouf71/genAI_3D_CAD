...
async def main():
    config = RAGAnythingConfig(
        working_dir="./rag_storage_ollama_bevel_gear",
        parser="mineru",
        parse_method="auto",
        ...
        # context_filter_content_types = ["table"]          
    ...
    async def ollama_llm_model(
        ...
        **kwargs,
        ...
        if system_prompt and "---Context---" in system_prompt:
            ...
            4) Search ONLY within the "table.rows" of the TABLE_AS_JSON block:
                -Find the row where row[column_name] EXACTLY equals the requested value as a string 
                (same digits, punctuation, e.g., "65" vs "65,0").
                - Do NOT pick “closest” or “similar” values. Exact match only.

            5)If you find the matching row:
                - OUTPUT ONLY the JSON for the matching row object.
                - Do NOT output any other rows or the entire table NO MATTER WHAT .
                - Preserve all keys and string values exactly as they appear in the table (same spelling, digits, commas, dots, and units).
                - DO NOT wrap the output in Markdown, tables, code fences, or any extra text.
                - Do NOT explain the result. Just output the raw JSON object.
                - If no matching row is found, simply output nothing (empty response).
    ...
    await rag.process_document_complete(
        ...
        output_dir="./output_ollama_bevel_gear",
        parse_method="auto",
    ...
    text_result = await rag.aquery(
        "From the table with dimensions for bevel gears made of zinc (ZnAl4Cu1), please extract the full table row where Art.-Nr. = KD1516-1:1ZN",
        vlm_enhanced=False,
        enable_rerank=False,
        chunk_top_k=9,      
        # mode="naive",
